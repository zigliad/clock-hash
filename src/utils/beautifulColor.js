...
export function isExactCssColor(r, g, b) {
...
export function isBeautiful(hex) {
  ...
  if (s > MIN_SATURATION && l >= MIN_LIGHTNESS && l <= MAX_LIGHTNESS) {
    return { type: 'high-saturation', name: 'high-saturation color' }
  }

  return null
}

export function timeStringToHex(timeStr) {
  const hex = timeStr.replace(/:/g, '')
  return /^[0-9a-fA-F]{6}$/.test(hex) ? `#${hex}` : '#000000'
}

export function secondsToTimeStr(totalSeconds, is24h) {
...
export function findNextBeautifulTime(currentSeconds, is24h) {
